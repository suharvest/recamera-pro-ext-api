            ...
            while not self._stop.is_set():
                ...
                if not ok or frame is None:
                    if self.spec.loop and not self.spec.is_live:
                        ...
                        try:
                            capture = self._capture_factory(self.spec)
                        except Exception as exc:  # noqa: BLE001 - 重开失败要报出来
                            # 不捕获的话异常会从采集线程里冒掉，stats() 里
                            # error 仍是 None，整轮 soak 会被当成"源正常放完"。
                            self.error = (f"{self.spec.stream_id}: reopen failed "
                                          f"after {self.loops} loop(s): {exc}")
                            break
                        self.loops += 1
                        continue
